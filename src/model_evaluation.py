...
#Load the model from models folder (model.pkl file)
def load_model(file_path: str):
    ...
    try:
        ...
        logger.error("Model not found: %s", file_path)
        raise
...
def load_data(file_path: str) -> pd.DataFrame:
    """
    Load data from a csv file processed (test_tdidf.csv)
    """
    try:
...
def save_metrics(metrics:dict, file_path:str) -> None:
    ...
    try:
        ...
        logger.error("Error occured while svaing the metrics: %s", e)
        raise
...
def main():
    try:
        ...
        logger.error("Failed to complete the model evaulation process: %s", e)
        print(f'Error:{e}')
